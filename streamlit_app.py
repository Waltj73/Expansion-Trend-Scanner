...
# -----------------------------------------------------------------------
# Returns heat map (sector x timeframe)
# -----------------------------------------------------------------------

st.subheader("🔥 Returns Heat Map")
st.caption(
    "Every sector's return across every timeframe at once. Read across a row to see whether a "
    "sector's strength (or weakness) is recent, sustained, or fading."
)

heatmap_universe = list(available_sectors.items()) + list(available_extras.items())
heatmap_labels = [f"{name.split(' (')[0]} ({t})" for t, name in heatmap_universe]

heatmap_data = []
for ticker, _ in heatmap_universe:
    row_vals = [compute_returns(prices[[ticker]], days)[ticker] for days in LOOKBACKS.values()]
    heatmap_data.append(row_vals)

heatmap_df = pd.DataFrame(heatmap_data, index=heatmap_labels, columns=list(LOOKBACKS.keys()))

# Sort rows by the same window currently used to rank the leaderboard, so the heat map
# and the table above tell the same top-to-bottom story.
heatmap_df = heatmap_df.sort_values(rank_lookback_label, ascending=False)

# Symmetric color scale around 0 so red/green intensity is comparable across the whole grid,
# not stretched by whichever single cell happens to be the most extreme. nan-safe in case any
# ticker is missing a data point for a given window.
_abs_vals = heatmap_df.abs().to_numpy(dtype=float)
max_abs = max(1.0, float(np.nanmax(_abs_vals)) if not np.all(np.isnan(_abs_vals)) else 1.0)

fig_heatmap = px.imshow(
    heatmap_df,
    color_continuous_scale=["#d73027", "#fee08b", "#ffffff", "#a6d96a", "#1a9850"],
    zmin=-max_abs,
    zmax=max_abs,
    aspect="auto",
    labels=dict(x="Timeframe", y="", color="Return (%)"),
    text_auto=".1f",
)
...
fig_heatmap.update_layout(
    height=max(320, 42 * len(heatmap_universe)),
